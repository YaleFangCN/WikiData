...
class GUI(QWidget):
    ...
    def initUI(self):
        ...
        self.resize( 820, 550)
        self.center()
        self.setWindowTitle('WikiData---Qs4 Properties-Search')
    ...
    def fun(self):
        ...
        sql2 = "SELECT description FROM web_main WHERE (entity_id like '%%%s%%')" % (value)
        #sql2 = "SELECT description FROM web_main WHERE entity_id = '%s'" % (value)
        cur.execute(sql2)
        propertys = str(cur.fetchall()).replace('\'', '').replace(',', '').replace('(', '').replace(')', '')
        con.commit()
        self.resultEdit.append(propertys)
